...
| 赛道 | 说明 |
|------|------|
| 📚 大模型八股 | 后训练/多模态/RAG/Agent 四专题知识考察，AI 出题+评分+参考答案，支持收藏 |
...
| 📄 论文精读 | 上传技术论文 PDF，AI 解析核心技术并出题考察，论文缓存到本地 |
| 📊 面试复盘 | 上传真实面试录音，Whisper 转录+AI 表现诊断 |
| ⭐ 收藏夹 | 收藏问题+参考答案，支持删除管理，多赛道统一收藏
...
| AI | DeepSeek v4 (flash/pro) + OpenAI Whisper（仅语音转录） |
| 编辑器 | Monaco Editor（手撕代码赛道） |
| 语音 | Web Speech API + Whisper |
| 缓存 | 本地文件系统 `.offerpilot_cache/` + localStorage 页面状态缓存 |
...
- **页面状态缓存**：页面离开时自动保存聊天/代码/审查状态到 localStorage，返回时完整恢复，每页配备刷新按钮
